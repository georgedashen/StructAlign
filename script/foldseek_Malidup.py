...
# enter the folder first Malidup
...
os.chdir('Malidup')
df = pd.read_table('../Malidup_result/aln.tsv',header=None)
...
for folder in folderlist:
    ...
    try:
        _,_,aln1,aln2=df[(df['Query']==q)&(df['Target']==t)].values[0]
    except:
    ...
    # run TM-align
    try:
        if not os.path.exists(f'{entry}.foldseek.tmalign') and record_accuracy:
    ...
    #result = subprocess.run(f"cat {entry}.foldseek.ali", stdout=subprocess.PIPE, shell=True, text=True)
